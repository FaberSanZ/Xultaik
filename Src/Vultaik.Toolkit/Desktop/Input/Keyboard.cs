...
{
    public class Keyboard : ISubscriber
    {
        ...
        public unsafe bool IsKeyPressed(Key key) => GlfwProvider.GLFW.Value.GetKey(_handle, ConvertKey(key)) is (int)InputAction.Press;

        public event Action<Keyboard, Key, int>? KeyDown;
        public event Action<Keyboard, Key, int>? KeyUp;
