...
namespace Xultaik.Graphics
{
    ...
    public unsafe class WICLoader
    {
        ...
        public WICLoader(string filename)
        {
        ...
        public static TextureData LoadFromFile(string filename) => new WICLoader(filename).TextureData;

        internal IWICBitmapSource ReadBitmap(string filename)
        {
            using var factory = new IWICImagingFactory2();

            IWICBitmapDecoder bitmapDecoder = factory.CreateDecoderFromFileName(filename, FileAccess.Read, DecodeOptions.CacheOnLoad);
