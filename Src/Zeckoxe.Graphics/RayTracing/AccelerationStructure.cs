...
namespace Zeckoxe.Graphics
{
	...
	public unsafe class AccelerationStructure : GraphicsResource
    {
		...
		internal RayTracingScratchBuffer create_scratch_buffer(VkAccelerationStructureKHR accelerationStructure)
		{
			...
			bufferCI.sType = VkStructureType.BufferCreateInfo;
			bufferCI.size = memoryRequirements2.memoryRequirements.size;
			bufferCI.usage = VkBufferUsageFlags.RayTracingKHR | VkBufferUsageFlags.ShaderDeviceAddress;
			...
			vkGetBufferMemoryRequirements(NativeDevice.handle, scratchBuffer.buffer, out memoryRequirements);
